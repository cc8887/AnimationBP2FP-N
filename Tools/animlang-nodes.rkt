...
;; 参数:
;;   :name         String  ;; 状态机名称
;;   :initial      String  ;; 默认起始状态
;;   :transitions  List    ;; 转换列表 [(from -> to :duration F :priority N :rule Expr) ...]
;; Pose 输入 (每个状态成为一个命名子节点):
;;   :state-name-kebab  AnimNode  ;; 该状态的动画子树
;;
;; 转换条件格式:
;;   :rule (ref "蓝图节点标题")        ;; 蓝图逻辑节点
;;   :rule (auto-rule :time-remaining crossfade-duration)  ;; 自动规则
;;   :rule (auto-rule :time-remaining 0.25)                ;; 自动规则 (指定时间)
;; 转换属性:
;;   :duration      Float    ;; 交叉淡入淡出时长 (默认 0.2)
;;   :priority      Integer  ;; 优先级 (数字小的优先)
;;   :bidirectional Boolean  ;; 双向转换
;;
;; 新增节点 (从状态内部发现):
;;   pose-snapshot  ;; 姿态快照 (通用回退导出)
(: state-machine (->* ()
                      (#:name String
                       #:initial String
                       #:transitions Any)
