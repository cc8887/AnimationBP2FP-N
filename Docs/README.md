...
**重要决策**: AnimBP2FP 和 MaterialBP2FP 是**独立项目**，不合并。

**原因**：

1. **不同的函数式纯度**
   - AnimBP2FP: ~70% 纯函数（状态机有副作用）
   - MaterialBP2FP: ~95% 纯函数（几乎完全确定性）

2. **独立的检查工具**
   - AnimBP: 状态转换验证、时序逻辑、姿态混合检查
   - MaterialBP: 像素着色验证、纹理采样、HLSL 类型安全

3. **不同的类型系统**
   - AnimBP: UE 自定义类型（Pose, BoneTransform, AnimSequence）
   - MaterialBP: HLSL 标准类型（float/float2/float3/float4）

4. **不同的测试策略**
   - AnimBP: 姿态对比（需要运行时，慢）
   - MaterialBP: 像素对比（静态渲染，快）

**共享基础设施**：
```
Common/
├── sexpp/          # S-expression 解析器（共享）
├── Racket/         # Linter/Formatter 工具链（共享）
└── UE-Editor/      # 编辑器集成模式（共享）

AnimBP2FP/          # 独立项目（本项目）
└── 状态机、时序逻辑专用工具

MaterialBP2FP/      # 独立项目（姊妹项目）
└── HLSL 类型、像素着色专用工具
```
